...
if st.session_state['user_role'] == 'technician':
    ...
    with st.sidebar.expander("Admin: Upload Manual", expanded=False):
        ...
        if uploaded_file and st.button("Process & Secure"):
            ...
            try:
                # 1. CHECKPOINTING: Find the last processed page in Pinecone to avoid duplicates
                last_processed_page = 0
                try:
                    pc = Pinecone(api_key=st.secrets["PINECONE_API_KEY"])
                    idx = pc.Index(INDEX_NAME)
                    
                    # Query for metadata of this specific file
                    response = idx.query(
                        vector=[0.0] * 768, 
                    ...
                    status_box.write(f"⚠️ Could not check previous progress. Assuming new file. ({str(e)})")
                ...
                st.info("Don't worry! Your progress up to the last batch was saved. Click 'Process & Secure' again to resume.")
